...
import Product from "../ProductsRange/Product/Product";
...
const SpotlightProductsRange = () => {
  ...
  return (
    <div
      style={{
        background: `
    url(/assets/svg/spotlightBig.svg) no-repeat, 
    linear-gradient(rgb(255, 248, 227) 0%, rgba(255, 255, 255, 0) 100%)
  `,
      }}
      className="py-10 px-5"
    >
      {/*   // backgroundImage: "url(/assets/svg/spotlightBig.svg)" , */}
      <h6 className="text-2xl font-semibold text-slate-700 ">
        In the Spotlight
      </h6>
          ...
          {products.map((data, idx) => {
            return (
              <SwiperSlide className="swiper-slide" key={idx}>
                <Link href={data?.imageLink} key={idx}>
                  <Product
                    id={data?.id}
...
export default SpotlightProductsRange;
