...
const DoubleProductsRange: React.FC<ProductsRangeProps> = ({
  title,
  subTitle,
}) => {
  const products = [
  ...
  return (
    <div className="productsContainer">
      ...
      <div className="productsContainer__box relative">
        <Swiper
        ...
        >
          {Array.from({ length: Math.ceil(products.length / 6) }).map(
            (_, slideIdx) => (
              <SwiperSlide className="swiper-slide" key={slideIdx}>
                <div className="grid grid-cols-2 md:grid-cols-3  gap-5">
                  {products
                    .slice(slideIdx * 6, slideIdx * 6 + 6) // Take 6 products for each slide
                    .map((data, idx) => (
...
export default DoubleProductsRange;
